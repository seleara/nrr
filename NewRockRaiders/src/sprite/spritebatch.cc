...
std::vector<std::pair<size_t, TextureResource *>> SpriteBatch::upload() {
	std::vector<std::pair<size_t, TextureResource *>> textures;
	...
	std::sort(sprites_.begin(), sprites_.end(), [](const SpritePair &a, const SpritePair &b) -> bool {
		...
		auto aid = ((GLTextureResource *)(a.first.texture.get()))->id();
		auto bid = ((GLTextureResource *)(b.first.texture.get()))->id();
	...
	int i = 0;
	const auto &windowSize = glm::ivec2(1280, 720);//Window::main().size();
	for (const auto &st : sprites_) {
		...
		glm::vec2 topLeft(origin.x + posMod.x * t.position.x - pivot.x, origin.y + posMod.y * t.position.y - pivot.y);
